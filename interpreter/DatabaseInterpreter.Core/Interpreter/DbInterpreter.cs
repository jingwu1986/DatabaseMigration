...
namespace DatabaseInterpreter.Core
{
    public abstract class DbInterpreter
    {
        ...
        private bool NeedFetchObjects(string[] names)
        {
            return (names != null && names.Any()) || this.Option.GetAllObjectsIfNotSpecified;
        }
        #endregion
        #endregion
        ...
        public virtual async Task SetIdentityEnabled(DbConnection dbConnection, TableColumn column, bool enabled) { }
            ...
            using (DbConnection connection = this.dbConnector.CreateConnection())
            {
                ...
                foreach (Table table in schemaInfo.Tables)
                {
                    ...
                    if (isSelfReference)
                    {
                        ...
                        dictPagedData = await this.GetSortedPageDatas(connection, table, primaryKeyColumns, parentColumnName, columns, strWhere);
                    }
                    else
                    {
        ...
        private async Task<Dictionary<long, List<Dictionary<string, object>>>> GetSortedPageDatas(DbConnection connection, Table table, string primaryKeyColumns, string parentColumnName, List<TableColumn> columns, string whereClause = "")
        {
            ...
            if (parentValues.Count > 0)
            {
                ...
                for (long parentValuePageNumber = 1; parentValuePageNumber <= parentValuesPageCount; parentValuePageNumber++)
                {
                    ...
                    if (total > 0)
                    {
                        Dictionary<long, List<Dictionary<string, object>>> dictChildPagedData = await this.GetSortedPageDatas(connection, table, primaryKeyColumns, parentColumnName, columns, whereClause);
