    ...
    public class OracleInterpreter : DbInterpreter
    {
        ...
        public override Task<List<Database>> GetDatabasesAsync()
        {
            ...
            if (!this.ShowBuiltinDatabase)
            {
                ...
                notShowBuiltinDatabaseCondition = string.IsNullOrEmpty(strBuiltinDatabase) ? "" : $"WHERE TABLESPACE_NAME NOT IN({strBuiltinDatabase}) AND CONTENTS <>'UNDO'";
            }

            string sql = $@"SELECT TABLESPACE_NAME AS ""Name"" FROM USER_TABLESPACES {notShowBuiltinDatabaseCondition}";
        ...
        private string GetSqlForProcedures(string type, params string[] objectNames)
        {
            ...
            string ownerCondition = $" AND UPPER(P.OWNER) = UPPER('{ConnectionInfo.UserId}')";
        ...
        private string GetSqlForTables(params string[] tableNames)
        {
            ...
            sql += $" WHERE UPPER(OWNER)=UPPER('{ConnectionInfo.UserId}')";
         ...
         private string GetSqlForTriggers(params string[] triggerNames)
        {
            ...
            string sql = $@"SELECT TRIGGER_NAME AS ""Name"",TABLE_OWNER AS ""Owner"", TABLE_NAME AS ""TableName"", 
                        ...
                        FROM USER_TRIGGERS
                        WHERE UPPER(TABLE_OWNER) = UPPER('{ConnectionInfo.UserId}')
                        ";
...
REFERENCES { this.GetQuotedString(tableForeignKey.ReferencedTableName)}({referenceColumnName})");
        ...
        {
            string sql = $@"SELECT COUNT(1) FROM {this.ConnectionInfo.UserId}.{ this.GetQuotedString(table.Name)}";
